...
process_ws_shake_hand(struct session* ses, struct io_package* io_data, 
	...
	send(ses->c_sock, accept_buffer, (int)strlen(accept_buffer), 0);
	ses->is_shake_hand = 1;
...
struct session*
gateway_connect(char* server_ip, int port) {
	...
	while (1) {
		...
		switch (io_data->opt) {
			case IOCP_RECV: { // 完成端口意味着数据已经读好
				...
				else if (socket_type == WEB_SOCKET_IO) { // 处理web socket,自己本省就定义了封包与解包的格式;
